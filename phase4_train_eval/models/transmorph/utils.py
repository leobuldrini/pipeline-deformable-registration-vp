...
def dice_val_labels(y_pred, y_true, labels):
    """Compute Dice for a given list of labels (numpy arrays)."""
    pred = y_pred if isinstance(y_pred, np.ndarray) else y_pred.detach().cpu().numpy()
    true = y_true if isinstance(y_true, np.ndarray) else y_true.detach().cpu().numpy()
...
def jacobian_determinant_vxm(disp):
    ...
    Parameters:
        disp: 2D or 3D displacement field of size [nb_dims, *vol_shape]
              (i.e., 3D: shape (3, D, H, W))
    Returns:
