...
# Shared losses from common/losses.py — same implementation as TransMorph/VoxelMorph.
# - VolumePreservationLoss: Dong et al. ICCV 2023 Eq. 10
# - MaskedNCC: Dong et al. ICCV 2023 Eq. 11-12 (soft-weighted global Pearson)
import os as _os, importlib.util as _ilu
_cl_path = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), '..', '..', '..', 'common', 'losses.py')
_cl_spec = _ilu.spec_from_file_location("_common_losses", _cl_path)
_cl_mod = _ilu.module_from_spec(_cl_spec)
_cl_spec.loader.exec_module(_cl_mod)
VolumePreservationLoss = _cl_mod.VolumePreservationLoss
MaskedNCC_Dong = _cl_mod.MaskedNCC  # Dong Pearson global with soft weight
compute_stsr = _cl_mod.compute_stsr  # Dong et al. ICCV 2023 evaluation metric
eval_stsr = _cl_mod.eval_stsr        # convenience wrapper for mask tensors
smooth_tumor_mask = _cl_mod.smooth_tumor_mask  # Brett 2001 mask smoothing
